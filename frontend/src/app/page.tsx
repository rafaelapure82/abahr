...
import { 
  QrCode, User, Clock, ArrowRight, CheckCircle2, 
    ...
    <main className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-4 overflow-hidden relative">
      ...
      <div className="z-10 text-center mb-12 animate-in fade-in slide-in-from-top-4 duration-1000">
        ...
        <p className="text-slate-400 text-xl font-medium">
          {mounted ? time.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }) : 'Cargando...'}
        </p>
      </div>
      ...
      <div className="z-10 w-full max-w-xl bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
        ...
        <div className="flex p-2 bg-black/20 m-6 rounded-2xl gap-2">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl transition-all duration-300 ${method === 'qr' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-white/5'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl transition-all duration-300 ${method === 'manual' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-white/5'}`}
          >
        ...
        <div className="p-8 pt-2 min-h-[400px] flex flex-col items-center justify-center relative">
          ...
          {message && (
            <div className={`absolute inset-0 z-50 ${message.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'} flex flex-col items-center justify-center p-8 text-center animate-in zoom-in duration-300`}>
              {message.type === 'success' ? <CheckCircle2 className="w-24 h-24 mb-6 animate-bounce" /> : <AlertCircle className="w-24 h-24 mb-6" />}

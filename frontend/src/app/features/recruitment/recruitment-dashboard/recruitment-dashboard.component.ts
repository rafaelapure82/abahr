        ...
        <app-card class="col-span-1">
          ...
          <app-card-content class="p-0">
            <div class="divide-y divide-border">
              @if (loading) {
                <div class="p-8 text-center text-muted-foreground italic text-sm">Cargando vacantes...</div>
              }
              @if (!loading && vacancies.length === 0) {
                <div class="p-8 text-center text-muted-foreground italic text-sm">No hay vacantes activas.</div>
              }
              @for (job of vacancies; track job) {
      ...
      <app-card>
        ...
        <app-card-content class="p-0">
          <table class="w-full text-sm">
            @for (candidate of recentCandidates; track candidate) {

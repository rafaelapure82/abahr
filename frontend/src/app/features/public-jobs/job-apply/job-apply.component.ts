...
@Component({
    ...
    imports: [RouterLink, FormsModule, ReactiveFormsModule, LucideAngularModule, CardComponent, CardHeaderComponent, CardTitleComponent, CardContentComponent, ButtonComponent],
    template: `
    <div class="min-h-screen bg-muted/30 pb-12 pt-8">
      <div class="max-w-2xl mx-auto px-6">
        ...
        @if (loadingJob) {
          <div class="text-center p-12 bg-card rounded-2xl border border-border">
              ...
              <app-card-content>
                <form [formGroup]="applyForm" (ngSubmit)="onSubmit()" class="space-y-6">
                  ...
                  <div class="space-y-2">
                    ...
                    @if (selectedFile) {
                      <div class="flex items-center gap-2 p-2 bg-primary/5 rounded border border-primary/20 text-xs text-primary">
                  ...
                  @if (success) {
                    <div class="p-4 bg-green-500/10 border border-green-500/20 text-green-600 rounded-lg text-sm flex items-center gap-3">
                      <lucide-icon name="check-circle" size="18"></lucide-icon>
                      ¡Postulación enviada con éxito! Te contactaremos pronto.
                    </div>
                  }
                  @if (error) {
                    <div class="p-4 bg-red-500/10 border border-red-500/20 text-red-600 rounded-lg text-sm">
                      {{ error }}
                    </div>
                  }

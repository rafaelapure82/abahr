...
@Component({
    ...
    >
      ...
      <div class="flex items-center h-24 px-8 overflow-hidden whitespace-nowrap relative">
        ...
        <div *ngIf="!isCollapsed" class="ml-4 relative z-10">
      ...
      <nav class="p-4 space-y-1.5 overflow-y-auto h-[calc(100vh-12rem)] no-scrollbar">
        <ng-container *ngFor="let item of navItems; let i = index">
          <!-- Section Header -->
          <p *ngIf="!isCollapsed && item.section" class="px-4 pt-6 pb-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
            {{ item.section }}
          </p>
          ...
          >
            ...
            <span *ngIf="!isCollapsed" class="ml-4 text-sm font-bold transition-all duration-300 whitespace-nowrap tracking-wide">
              {{ item.label }}
            </span>

            <!-- Badge -->
            <span *ngIf="!isCollapsed && item.badge" class="ml-auto text-[9px] font-black px-2 py-0.5 rounded-lg bg-indigo-500/20 text-indigo-400 uppercase border border-indigo-500/20">
              {{ item.badge }}
            </span>
      ...
      <div class="absolute bottom-0 w-full p-4 space-y-4">
        <div *ngIf="!isCollapsed" class="glass-panel p-4 rounded-3xl border border-white/5 bg-white/5 flex items-center gap-3">
        ...
        >
          ...
          <span *ngIf="!isCollapsed" class="ml-3 text-sm font-black uppercase tracking-widest">Contraer</span>
        </button>
      </div>
    </aside>
  `,
